...
n_paramval = 30 # the number of generated parameter values for each dataset
n_dataval = 8 # the number of generated data values for each dataset
n_sample = 1000 # the number of posterior samples for each dataset (nchains * ndraws)

generator <- function(hyperparams, paramvals, predictor = NULL){
  # hyperparamter
  n_dataval = hyperparams$n_dataval
  n_paramval <- dim(paramvals[[1]])
  theta_loc <- rfun(rnorm, ndraws = 1) (n_paramval, rvar_mean(paramvals$theta), 0) #rvar<1>[n_paramval]
  theta_scale <- rfun(rnorm, ndraws = 1) (n_paramval,rvar_sd(paramvals$theta), 0)
  # parameter
  theta <- rfun(rnorm, ndraws = 1)(n = n_paramval, mean = theta_loc, sd = theta_scale) 
  # predictor
  # response
  y <- rfun(rnorm, ndraws = n_dataval)(n = n_paramval, mean = paramvals$theta, sd = 1) 
...
datasets <- generator(
    hyperparams = list(n_dataval = n_dataval),
  	paramvals = draws_rvars(theta = rfun(rnorm, ndraws = 1) (n = n_paramval, mean = 0, sd = 1))
)
backend <- SBC_backend_cmdstan_sample(mod_simple_normal, iter_sampling = n_sample)
...
With zero mean for x, datasets from all `prior_width` are good. Generator input consists of true parameter point values `ture_priorvals` and `predictor`. Note the difference in plural and singular. Compared to parameters which has variability between each prior values (rvar<n_paramval>[1], e.g.0.026 ± 0.99), predictor's variability exist only within one datasets. All dataset from each priorpoint have the same value of predictor (rvar<1>[n_paramval]), e.g. 1.4724 ± NA).
...
generator_sgmd <- function(hyperparams, paramvals, predictor){
  # hyperparamter
  n_dataval = hyperparams$n_dataval
  n_paramval <- ndraws(paramvals)
  sigma = hyperparams$sigma
  prior_width = hyperparams$prior_width
  # paramter
  w = paramvals$w #n_paramval vector
  b = paramvals$b
  # predictor
  x = predictor$x 
  # generate
  ...
  y <- rfun(rnorm, ndraws = n_dataval)(n = n_paramval, mean = y_true, sd = sigma) 
  gen_rvars <- draws_rvars(N = n_paramval, x = x, y = y, prior_width = prior_width, sigma = sigma)
  SBC_datasets(
    parameters = as_draws_matrix(paramvals), 
    generated = draws_rvars_to_standata(gen_rvars)
  )
...
datasets_sgmd_x0 <- generator_sgmd(
	paramvals = draws_rvars(w = rvar(rnorm(n_paramval, 0, 1)), b = rvar(rnorm(n_paramval, 0, 1))), 
	predictor = draws_rvars(x = rfun(rnorm, ndraws = 1) (n = n_paramval, 0, 2)), 
	hyperparams = list(prior_width = 10, sigma = 0.1, n_dataval = 10),
	)
...
datasets_sgmd_x5_init <- generator_sgmd(
	paramvals = draws_rvars(w = rvar(rnorm(n_paramval, 0, 1)), b = rvar(rnorm(n_paramval, 0, 1))), 
	predictor = draws_rvars(x = rfun(rnorm, ndraws = 1) (n = n_paramval, 5, 2)), 
	hyperparams = list(prior_width = 10, sigma = 0.1, n_dataval = 10),
	)
...
```{R}
set.seed(1954)
datasets_sgmd_x5_w30 <-  generator_sgmd(
	paramvals = draws_rvars(w = rvar(rnorm(n_paramval, 0, 1)), b = rvar(rnorm(n_paramval, 0, 1))), 
	predictor = draws_rvars(x = rfun(rnorm, ndraws = 1) (n = n_paramval, 5, 2)), 
	hyperparams = list(prior_width = 30, sigma = 0.1, n_dataval = 10),
	)
...
predictor <- draws_rvars(x = rfun(rnorm, ndraws = 1) (n = n_paramval, 5, 2))
priors_sgmd_x5_sc <- selfCalib(generator_sgmd, priors_sgmd_x5_init, predictor, hyperparams, mod_sgmd, tv, n_dataval, n_sample, cnt = 1, evolve_df, delivDir)
datasets_sgmd_x5_sc <- generator_priorvals_sgmd(priors_sgmd_x5_sc, n_dataval, predictor) 
