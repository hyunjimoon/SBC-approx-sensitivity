...
n_prival = 3 # the number of generated parameter values for each dataset
...
datasets_simple <- generator_priorvals_simple(draws_rvars(theta = rvar_rng(rnorm, 1, mean = 3, sd = 4, ndraws = n_prival)), n_dataval) #(n, ndraws) = (1, n_prival) for params, (n_prival, 1) for predictors
...
predictor = NULL 
sc_priors <- selfCalib(priors, generator_priorvals_simple, predictor, mod_simple_normal, tv, n_dataval, n_sample, cnt = 1, evolve_df, delivDir)
...
ggsave(sc_g, file =  file.path(delivDir, paste0(paste0(paste0(tv, "_"), "afCalib_ecdf.png"), sep = "")))
# numeric inspection
priors_next <- adj_post(priors, result_simple, tv, sumtype = "MtoM") 
sc_priors_next <- adj_post(sc_priors, sc_result_simple, tv, sumtype = "MtoM")
self_cons_msr_bfcalib <- set2set(priors, priors_next, "theta")                                                
self_cons_msr_afcalib <- set2set(sc_priors, sc_priors_next, "theta")
csv_save(list(before = unlist(self_cons_msr_bfcalib), after = unlist(self_cons_msr_afcalib)), "theta", delivDir, type = "diagnositcs")
```
...
set.seed(2000)
datasets_sgmd_x5_init <- generator_priorvals_sgmd(true_params = draws_rvars(w = rvar(rnorm(n_priorval, 0, 1)), b = rvar(rnorm(n_priorval, 0, 1))), n_dataval, predictor = draws_rvars(x = rvar_rng(rnorm, n_priorval, 5, 2, ndraws = 1)))
...
```{R}
set.seed(1954)
generator_priorvals_sgmd_w <- function(true_params, n_dataval, predictor, priorwidth){
  n_priorval = ndraws(true_params)
  x = predictor$x 
  sigma = rvar(rep(0.1, n_priorval))
  w = true_params$w 
...
datasets_sgmd_x5_w1 <- generator_priorvals_sgmd_w(true_params = draws_rvars(w = rvar(rnorm(n_priorval, 0, 1)), b = rvar(rnorm(n_priorval, 0, 1))), n_dataval,
                                                  predictor = draws_rvars(x = rvar_rng(rnorm, n_priorval, 5, 2, ndraws = 1)), priorwidth = 1)
datasets_sgmd_x5_w30 <- generator_priorvals_sgmd_w(true_params = draws_rvars(w = rvar(rnorm(n_priorval, 0, 1)), b = rvar(rnorm(n_priorval, 0, 1))), n_dataval, 
                                                   predictor = draws_rvars(x = rvar_rng(rnorm, n_priorval, 5, 2, ndraws = 1)), priorwidth = 30)
...
priors_sgmd_sc <- selfCalib(priors_sgmd_init, generator_priorvals_sgmd, predictor, mod_sgmd, tv, n_dataval, n_sample, cnt = 1, evolve_df, delivDir)
...
# numeric inspection
priors_next <- adj_post(priors, result_sgmd_x5_sc, "w", sumtype = "MtoM") 
priors_next_sc <- adj_post(priors_sgmd_sc, result_sgmd_x5_sc, tv, sumtype = "MtoM")
