...
par = "theta"
var = "theta" # affects iter_stop related to decision variable/objective fucntion
D <- 8
...
workflow$fit_model(sample_iterations = M, warmup_iterations = 500, data)
prior <- workflow$prior_samples
post <- workflow$posterior_samples
rank <- workflow$calculate_rank()
...
sc_workflow <- SBCWorkflow$new(simple_normal, generator_np())
sc_workflow$simulate(n_sbc_iterations = N, as_draws_df(sc_prior)[[pars]])
data$theta_loc = mean(sc_prior[[pars]])  
data$theta_scale = sd(sc_prior[[pars]])
sc_workflow$fit_model(sample_iterations = M, warmup_iterations = 500, data)
...
g <- plot_ecdf_diff(workflow, K = M, var = var)
sc_g <- plot_ecdf_diff(sc_workflow, K = M, var = var)
...
bf <- rank_summary(as_draws_matrix(workflow$calculated_ranks), par, bins = 20)
af <- rank_summary(as_draws_matrix(sc_workflow$calculated_ranks), par, bins = 20)
csv_save(list(bf, af),delivDir, type = "diagnositcs")

```

# 2. $p_d$ effect: linear regression with two coefficient parameters with distribution
```{R}
modelName = "linreg"
modDir <- set_get_Dir(modelName)$modDir
delivDir <- set_get_Dir(modelName)$delivDir
linreg <- cmdstanr::cmdstan_model(set_get_Dir(modelName)$file)
generator <- function(){  
    function(){
    theta <- rnorm(1, 0, 1)
    list(
...
data <- list("N"= 25, "X" =  c(0.77464788,   7.05392102,  -2.6452878 ,   0.14542036,
                                -1.6399529 ,   0.92137426,   1.80403187,  -1.14307597,
                                -2.15443654,   3.97280007,  -6.82685002,   1.95364907,
                                3.81219001,  -0.43716175,   4.82478476,  -0.70158226,
                                2.41840505,  11.30962791,   0.30369471,  -7.81711537,
                                0.16801087,  -3.56495189,  -3.82249219,  -0.41908705,
                                -10.23615458, "Y" = y <- rep(0, D), "beta_loc"= 0, "beta_scale"= 1))
generator <- function(){
  function(){
    alpha ~ normal(1,0, 1);
...
#3. $p_p$ effect Normal-Normal conjugate, unkown mean (theta) and known sd(sigma)
...
# 4. $p_d$ effect: hierarchical model and parameterization

```{R}
modelName = "eightschools_ncp"
modDir <- set_get_Dir(modelName)$modDir
delivDir <- set_get_Dir(modelName)$delivDir
ncp_model = cmdstanr::cmdstan_model(set_get_Dir(modelName)$file)
generator <- function(){
  function(J){
    mu <- rnorm(1, 0, 5)
    tau <- rcauchy(1, 0, 5)
...
post <- workflow$posterior_samples
for (par in c("mu","tau")){
  plot_diag(workflow, par, bins = 20, delivDir)
}
