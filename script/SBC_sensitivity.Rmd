---
title: "using Cov from sample(=derivate of E_post(g(theta)) to set pre-SBC prior boundary" 
...
sbc <- function(stanmodel, modelName, data, N, M, n_eff_reltol=0.2, ..., save_progress, load_incomplete=FALSE) {
  ...
  for(n in 1:N) {     
    ...
    out <- stanmodel$sample(data, chains = 1, iter_warmup = 500, iter_sampling = 297, parallel_chains = 1, adapt_delta = 0.8,  save_warmup = FALSE, thin = 1) 
    print(out$summary())  
    post[[n]] = out
    #save
    if (doSave) {
...
plot.sbc <- function(x, modelName, thin = 3, ...) {
  ...
  suppressWarnings(ggplot2::ggplot(d, aes(x = u)) +
   ...
   ggsave(paste0(paste0("./plots/", modelName),  ".png"), width = 5, height = 5)
   suppressWarnings(ggplot2::ggplot(d, aes(x = u)) +
                     geom_histogram(binwidth = binwidth, color = "black",
                                    fill = "#ffffe8", boundary = 0) +
                     facet_wrap(vars(parameter)) +
...
uniformity.sbc <- function(x, modelName, bins = 20, thin = 3){
  ...
  for (i in 1:num_params){
    ...
    integ_msr[i] <- dist(rbind(rep(bin_size, bins), bin_count), method = "manhattan") / M
  ...
  print("integ_msr")
  print(integ_msr)
  write.csv(list("pval" = pval,"bin_counts"=  bin_counts, "integ" = integ_msr), file = paste0(paste0("./csv/", modelName),  ".csv"))
  return(list(bin_counts = bin_counts, pval = pval, integ_msr = integ_msr))
}
...
data generated with soverdispersed model 

```{r}
data = list()
modelName <- "t_norm_0mu"
file <- file.path(modelDir, modelName, paste0(modelName, ".stan"))
mod <- cmdstan_model(file)
...
for (hp_sd in 1:5){
  data$hp_sd = hp_sd
  submodelName = paste0(modelName, hp_sd)
...
```{r}
data = list()
modelName <- "nb_pois_lambda"
file <- file.path(modelDir, modelName, paste0(modelName, ".stan"))
mod <- cmdstan_model(file)
N = 10 #(bins = 20)*N # num of sim and fit
M = 297 # 897 (6n -3) # num of post. draw - related to iter_sampling - not used currently
delivDir <- set_Model_Dir(modelName)$delivDir
for (hp_lambda in 1:5){
  data$hp_lambda = hp_lambda
  submodelName = paste0(modelName, hp_lambda)
  sbc_res_nb_pois_lambda <- sbc(mod, submodelName, data = data, N = N, M = M, save_progress = delivDir)
  uniformity.sbc(sbc_res_nb_pois_lambda)
  plot.sbc(sbc_res_nb_pois_lambda, submodelName)
}
```
...
$$
\begin{aligned} \tilde{\theta} & \sim \pi_{\mathrm{S}}(\theta) \\ \tilde{y} & \sim \pi_{\mathrm{S}}(y \mid \tilde{\theta}) \\\left(\tilde{\theta}_{1}^{\prime}, \ldots, \tilde{\theta}_{L}^{\prime}\right) & \sim \pi(\theta \mid \tilde{y}) \end{aligned}
$$
$$\rho=\#\left\{\tilde{\theta}<\tilde{\theta}_{l}^{\prime}\right\}$$
$$\rho_1, ... , \rho_N$$
$$g(\theta) = χ^2 test-pval $$

$$E[g(θ)] = ∫ dθ π_S (θ ∣ \tilde{y}) g(θ)$$
