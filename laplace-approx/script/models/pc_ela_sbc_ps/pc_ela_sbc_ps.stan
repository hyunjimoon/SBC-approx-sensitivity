...
data {
  int<lower=0> n_obs;				      // number of observations - d
  int<lower=0> n_covariates;             // number of predictors - p
...
transformed data {
  ...
  vector[n_covariates] beta_  =  z_ .* lambda_tilde_ * tau_;              // regression coefficients

  vector[n_covariates + 1] parm_;        // regression coefficients including icept
  parm_[1] = tau_;
  parm_[2:(n_covariates + 1)] = lambda_tilde_;
  vector[n_obs] p_ = inv_logit(laplace_approx_bernoulli_rng(y, n_samples, K_functor, parm_, X, delta, delta_int, theta0));
...
model {
  ...
  target += laplace_marginal_bernoulli(y, n_samples, K_functor, parm, X, delta, delta_int, theta0);
}
...
generated quantities {
  ...
  pars_[4] = p40;
  pars_[5] = p50;
  vector[n_obs] p = inv_logit(laplace_approx_bernoulli_rng(y, n_samples, K_functor, parm, X, delta, delta_int, theta0));
  int ranks_[n_par] = {p[10] < p_[10], p[20] < p_[20],p[30] < p_[30] ,p[40] < p_[40],p[50] < p_[50]};
}
